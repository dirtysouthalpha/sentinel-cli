  ...
  return {
    ...
    execute: async (args): Promise<ToolResult> => {
      try {
        ...
        const filePath = resolve(root, args.path as string);
        // Separator-aware containment check so a sibling dir sharing a name
        // prefix (e.g. `<root>-secrets`) is not treated as inside the project.
