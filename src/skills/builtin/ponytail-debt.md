...
<!-- Adapted from https://github.com/DietrichGebert/ponytail (MIT, DietrichGebert).
     In Sentinel, run the sweep with the `search` tool (regex mode), pattern
     `(\/\/|#|;)\s?ponytail:` across the repo, excluding node_modules/dist/.git. -->
...
Sweep the repo for comment markers, skipping `node_modules`, `dist`, `.git`,
and other build output:

`search` (regex) for `(\/\/|#|;|\/\*) ?ponytail:` — add other comment
prefixes if your stack uses them.
...
`git blame -L<line>,<line>` via the `bash` tool.
