...
export async function installEntry(
  ...
  if (entry.type === "skill") {
    ...
    mkdir(skillsDir);
    // `entry.id` comes from a remote registry — sanitize before using it as a
    // filename so an id like `../../CLAUDE` can't escape the skills dir and
    // overwrite arbitrary `.md` files (e.g. CLAUDE.md → persistent prompt injection).
    const safeId = entry.id.replace(/[\\/]/g, "_").replace(/\.\./g, "_");
    const dest = join(skillsDir, `${safeId}.md`);
